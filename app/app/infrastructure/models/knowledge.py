...
class KnowledgeIngestionJob(UUIDMixin, TimestampMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "idempotency_key", name="uq_knowledge_ingestion_idempotency_key"
        ),
        Index(
    ...
    profile_key: Mapped[str] = mapped_column(
        String(80), nullable=False, default="markdown"
    )
    ...
    source_artifact_refs: Mapped[list[dict]] = mapped_column(
...
class KnowledgeDocumentVersion(UUIDMixin, TimestampMixin, Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "ingestion_id", name="uq_knowledge_document_version_ingestion"
        ),
        Index("ix_knowledge_version_dataset_status", "dataset_key", "status"),
        Index(
            "ix_knowledge_version_provider_document", "provider", "provider_document_id"
        ),
        Index(
