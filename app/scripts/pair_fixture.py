...
def _principal(surface: str, *, operator: bool = False) -> Principal:
    ...
    return Principal(
        ...
        roles=roles,
        scopes=frozenset({"knowledge:admin"}) if surface == "admin" else frozenset(),
...
async def admin_me(
    session_id: str | None = Cookie(
        default=None, alias="sunmoonai_knowledge_admin_sid"
    ),
...
async def web_me(
    session_id: str | None = Cookie(default=None, alias="sunmoonai_knowledge_web_sid"),
) -> Response | dict[str, object]:
...
async def logout(
    ...
    response.delete_cookie(f"sunmoonai_knowledge_{surface}_sid", path="/")
    return response
